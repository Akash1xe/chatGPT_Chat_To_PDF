...
(() => {
  ...
  function buildTurnHtml(turn, options, index, questionNumber) {
    ...
    const label = role === 'assistant' && options.showModelName
      ? escapeHtml(options.modelName || 'ChatGPT')
      : baseLabel;
    ...
    return `
<section id="pdf-turn-${turn.number ?? index + 1}" class="pdf-turn pdf-turn-${role}${breakClass}" data-pdf-role="${role}">
...
@media print{.pdf-turn-user .pdf-turn-content,pre,blockquote,th{-webkit-print-color-adjust:exact;print-color-adjust:exact}}
  ...
  async function exportTurns(turns, overrides = {}, onStatus = () => {}) {
    ...
    if (!options.pdfcrowdUsername || !options.pdfcrowdApiKey) throw new Error('PDFCrowd credentials are missing. Open extension Options and add your username and API key.');
    ...
    const response = await chrome.runtime.sendMessage({
      type:'PDF_PROCESS', sessionId, filename,
      config:{ username:options.pdfcrowdUsername, apiKey:options.pdfcrowdApiKey, pageSize:options.pageSize || 'a4', orientation:options.orientation || 'portrait', singlePage:Boolean(options.singlePage) }
    });
